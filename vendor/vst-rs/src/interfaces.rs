...
pub fn host_dispatch(
    ...
    match opcode {
        ...
        // ...
        Ok(OpCode::CanDo) => {
             return host.can_do(&read_string(ptr));
        }
        ...
        Ok(OpCode::GetCurrentProcessLevel) => return host.get_process_level() as isize,
        Ok(OpCode::SizeWindow) => {
            let width = index;
            let height = value as i32;
            host.set_size(width, height);
        }
        Ok(OpCode::IOChanged) => host.io_changed(),
        _ => {
            println!("VST: Got unimplemented host opcode ({:?})", opcode);
            println!(
